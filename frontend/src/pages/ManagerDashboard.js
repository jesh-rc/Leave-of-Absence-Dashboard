...
function RequestsTable({ requests, showActions = false, onUpdateStatus }) {
  ...
  return (
    ...
    >
      <thead>
        <tr>
          <th style={{ borderBottom: "1px solid #ccc", textAlign: "left" }}>RID</th>
          <th style={{ borderBottom: "1px solid #ccc", textAlign: "left" }}>
            Employee
          </th>
          <th style={{ borderBottom: "1px solid #ccc", textAlign: "left" }}>CID</th>
          <th style={{ borderBottom: "1px solid #ccc", textAlign: "left" }}>
            Department
          </th>
          <th style={{ borderBottom: "1px solid #ccc", textAlign: "left" }}>
  ...
  async function handleUpdateStatus(rid, newStatus, currentStatus) {
    try {
      ...
      await apiFetch(`/leave_requests/${rid}`, {
        ...
        body: JSON.stringify({
          status: statusToSet,
          approvedby: user?.eid, // manager’s EID as decision maker
        }),
      });
      await loadRequests();
    } catch (err) {
